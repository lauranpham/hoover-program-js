...
function inputValidity(gridSize, hooverLocation, patchLocations, hooverMovements) {
  ...
  patchLocations.forEach((patch) => {
    if (patch.x >= gridSize.x || patch.y >= gridSize.y) {
      patch.valid = false;
      throw new Error(`The dirt patch at (${patch.x},${patch.y}) is outside your room dimensions. Please input a new location within the room.`);
    };
  });
...
function terminalOutput(hooverLocation) {
